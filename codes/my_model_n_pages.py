...
def load_data(fname):
	...
	for i in range(0, len(usid_list)-2):
		...
		if usid_list[i] != usid_list[i+1]:
			if pv > n_pages-1:
				# print new_usid_list[-pv:]
				# x = raw_input()
				pv = 0
			else :
				# print new_usid_list[-pv:]
				# x = raw_input()
				del new_usid_list[-pv:]
				del new_page_type_list[-pv:]
				del new_is_cv_list[-pv:]
				del new_dwell_list[-pv:]
				pv = 0
...
def make_learing_data(usid_list, page_type_list, usid_num_dict, I):
	...
	for i in range(0, len(usid_list)-1):
		...
		if usid_list[i] != usid_list[i+1]:
			...
			# use first 5 transitions
			# print ptype_seq
			# x= raw_input()
			for j in range(0, n_pages-1):
...
def main(fname):
	...
	fp_a = open("%s_pages_my_a_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
	csv_writer = csv.writer(fp_a)
	for item in np.a:
		csv_writer.writerows(item)

	fp_pi = open("%s_pages_my_pi_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
	csv_writer = csv.writer(fp_pi)
	csv_writer.writerow(np.pi)

	fp_lambda = open("%s_pages_my_lambda_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
	csv_writer = csv.writer(fp_lambda)
	csv_writer.writerows(np.Lambda)

	fp_c = open("%s_pages_my_c_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
	csv_writer = csv.writer(fp_c)
	csv_writer.writerow(np.c)


	fp_alpha = open("%s_pages_my_alpha_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
	...
	fp_features = open("%s_pages_my_features_%s_s=%s.csv" % (str(n_pages), fname, str(S)), "wb")
